...
import { Store, ShoppingCart, User, Search, CheckCircle, Headphones, Mail, ShieldAlert, Filter, ChevronRight, ChevronUp, PlayCircle, CircleDollarSign, ArrowLeft, Trash2, LogOut, Plus, Minus, Share2, Copy, ExternalLink, Wallet, Zap, Clock, Info, ShieldCheck, RefreshCcw, ArrowUpDown, CreditCard, History, Settings, LayoutDashboard, Eye, EyeOff, X, Download, MapPin, Shield, Database, Users, TrendingUp, AlertTriangle, AlertCircle, Smartphone, Package, PackageX, DollarSign, Activity, FileText, Trash, MessageCircle, Send, MessageSquare, Upload, Save, Edit, Hash, Sun, Moon, RotateCcw, Ban, UserCheck, Calendar, ShoppingBag, Bell, Menu } from 'lucide-react';
  ...
  <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 sticky top-0 z-50 font-sans">
    <div className="max-w-7xl mx-auto px-3 md:px-6 py-3 md:py-4 flex items-center justify-between gap-2 md:gap-6">
      ...
      <nav className="hidden lg:flex items-center gap-8">
        ...
        {session && (
          <button onClick={() => navigate('dashboard')} className={linkCls(currentView === 'dashboard' || currentView === 'proxies')}>{t('myOrders')}</button>
        )}
      </nav>
    ...
    {isMobileMenuOpen && (
      <div className="lg:hidden absolute top-full left-0 w-full bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 shadow-xl z-50 animate-in slide-in-from-top-2">
        <div className="flex flex-col py-4 px-6 gap-4">
          ...
          {session && (
            <button onClick={() => { setIsMobileMenuOpen(false); navigate('dashboard'); }} className={`text-left p-2 rounded-lg ${currentView === 'dashboard' || currentView === 'proxies' ? 'bg-primary/10 text-primary' : 'text-gray-600 dark:text-gray-300'}`}>{t('myOrders')}</button>
          )}
